...
function updateClock(){const now=new Date();const fmt={timeZone:'Asia/Jakarta',hour12:false};const dateFmt={timeZone:'Asia/Jakarta',weekday:'long',day:'2-digit',month:'long',year:'numeric'};const time=now.toLocaleTimeString('id-ID',fmt);const date=now.toLocaleDateString('id-ID',dateFmt);const a=document.getElementById('dashboardTime');if(a)a.textContent=time;const b=document.getElementById('liveClock');if(b)b.textContent=time;const c=document.getElementById('dashboardDate');if(c)c.textContent=date;const d=document.getElementById('dashboardTimeLarge');if(d)d.textContent=time;const e=document.getElementById('dashboardDateLarge');if(e)e.textContent=date}setInterval(updateClock,1000);updateClock();
...
function adminRender(tab='overview'){
 ...
 if(tab==='overview')c.innerHTML=`<div class="admin-welcome"><div><span class="eyebrow">XI TKJ 1 • ADMIN COMMAND CENTER</span><h3>Control everything. Keep the class moving.</h3><p>Kelola aktivitas, data siswa, tugas, materi, absensi, dan informasi kelas dari satu ruang kerja.</p></div><div class="admin-live-badge"><i></i> SYSTEM ONLINE</div></div><div class="admin-cards"><div class="admin-mini"><small>Siswa</small><b>${window.__CLASS_ACCOUNTS?.length||36}</b><span>anggota kelas</span></div><div class="admin-mini"><small>Tugas aktif</small><b>${tasks.filter(t=>t.status!=='done').length}</b><span>perlu dipantau</span></div><div class="admin-mini"><small>Saldo kas</small><b>${money(cash.balance)}</b><span>tersimpan lokal</span></div></div><div class="admin-quick-actions"><button onclick="adminRender('tasks')"><b>📝</b><span>Tugas</span><small>Kelola pekerjaan</small></button><button onclick="adminRender('materials')"><b>📚</b><span>Materi</span><small>Knowledge base</small></button><button onclick="adminRender('attendance')"><b>✓</b><span>Absensi</span><small>Status hari ini</small></button><button onclick="adminRender('accounts')"><b>👥</b><span>Siswa</span><small>Manage account</small></button></div><div class="admin-overview-note"><span>⌘</span><div><b>Ruang kontrol XI TKJ 1</b><p>Perubahan disimpan pada perangkat ini. Gunakan Export backup untuk menyimpan salinan konfigurasi sebelum melakukan perubahan besar.</p></div><button class="mini-btn" onclick="exportClassBackup()">Export backup</button></div>`;
...
(()=>{
 const update=()=>{
   ...
   ['dashboardTime','liveClock','dashboardTimeLarge'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=time});['dashboardDate','dashboardDateLarge'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=date});
